...
QFile timeStampFile("timestamp.csv");
QFile stableFile("stablelog.csv");
QTextStream timeStampOut(&timeStampFile);
QTextStream stableFileOut(&stableFile);
    ...
    ui(new Ui::MainWindow)
{
    ...
    m_controller->set_mode(1);
    server = new QTcpServer();
    server->listen(QHostAddress::Any, 6665);
    //connect(server, SIGNAL(newConnection()), this, SLOT(server_New_Connect()));
    //connect(this, SIGNAL(gt_changed()), this, SLOT(set_value()));
    //sconnect(socket, SIGNAL(readyRead()), this, SLOT(socket_Read_Data()));
    ...
    timeStampFile.open(QIODevice::Append);
    stableFile.open(QIODevice::Append);
...
void MainWindow::processData()
{
    ...
    qDebug()<<demoParams.totalPayloadSize_bytes<<endl;
...
//找关键字
        ...
        else
        {
            ...
            dataBuffer = dataBuffer.remove(0, dataStartIndex);
                  qDebug()<<"dataBuffer size after removal is "<<data.size();
        ...
        if (MagicOk == 1)
        {
            ...
            if(FileSavingFlag)
            {
                ...
                timeStampOut << NetworkTimestamp::shareInstance()->currentMSTimestamp() << endl;
            }
            ...
            //if (m_controller->mode!=0) m_controller->add_data(heartWfm_Out);
            // Range Profileffffffffffffffffffffff
            unsigned int numRangeBinProcessed = demoParams.rangeBinEnd_index - demoParams.rangeBinStart_index + 1;
            qDebug()<<numRangeBinProcessed;
            ...
            m_controller->set_range_parameters(demoParams.rangeBinSize_meters, demoParams.rangeStartMeters, numRangeBinProcessed);
            ...
            std::string tmpstr = m_controller->calculate_next(RangeProfile);

            if(FileSavingFlag)
            {
                if(tmpstr == "start") {
                    stableFileOut << "start," << NetworkTimestamp::shareInstance()->currentMSTimestamp() << endl;
                } else if(tmpstr == "stop") {
                    stableFileOut << "stop," << NetworkTimestamp::shareInstance()->currentMSTimestamp() << endl;
                }
            }
...
void MainWindow::socket_Read_Data()
{
    ...
    if(!buffer.isEmpty()) {
        gt_hr = std::stoi(buffer.toStdString());
        qDebug()<<gt_hr;
    }
}
...
void MainWindow::set_value(){
    QString myString_HeartRate;
    myString_HeartRate.sprintf("%1.0f",gt_hr);
    ui->lcdNumber_HeartRate->display(myString_HeartRate);
}
